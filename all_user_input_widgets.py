...
class Counter_program():
    ...
    def create_widgets(self):
        ...
        my_spinbox = tk.Spinbox(entry_frame,
                                from_=0, to=10, width=5, justify=tk.RIGHT)
        my_spinbox.grid(row=3, column=2, sticky=tk.W, pady=3)

        my_scale = tk.Scale(entry_frame, from_=0, to=100,
                            orient=tk.HORIZONTAL,
                            width=8, length=200)
        my_scale.grid(row=4, column=2, sticky=tk.W)

        my_scale = ttk.Scale(entry_frame, from_=0, to=100,
                             orient=tk.HORIZONTAL,
                             length=200)
    ...
    # - - - - - - - - - - - - - - - - - - - - -
        ...
        fromlist_frame = ttk.LabelFrame(self.window,
                                        ...
                                        relief=tk.RIDGE)
        fromlist_frame.grid(row=1, column=2,
                            sticky=tk.E + tk.W + tk.N + tk.S, padx=6)
        ...
        my_combobox = ttk.Combobox(fromlist_frame,
                                   height=4, textvariable=self.combobox_value)
